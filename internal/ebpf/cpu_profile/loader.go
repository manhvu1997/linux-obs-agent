...
func (l *Loader) Start(ctx context.Context) error {
	...
	// Attach to every online CPU via perf_event.
	...
	for cpu := 0; cpu < numCPU; cpu++ {
		...
		lnk, err := link.AttachPerfEvent(link.PerfEventOptions{
			Program: l.objs.ProfileCpu,
			PerfFd:  int(fd),
		})
		if err != nil {
			unix.Close(int(fd))
			slog.Warn("cpu_profile: attach failed", "cpu", cpu, "err", err)
			continue
		}
	...
	slog.Info("cpu_profile: started", "cpus", len(l.links), "hz", l.sampleHz)
	go l.consume(ctx)
	return nil
}
